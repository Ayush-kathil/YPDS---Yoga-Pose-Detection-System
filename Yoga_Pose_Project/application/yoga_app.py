...
class YogaPoseApp:
    ...
    def compare_poses(self, user_landmarks, reference_pose):
        """
        Compares poses using a weighted scoring system and provides specific feedback.
        """
        total_score, total_weight = 0, 0
        # This will store feedback for the joint with the biggest error
        worst_angle_feedback = {"name": "", "diff": -1}
        
        for ref_angle in reference_pose['landmarks']:
            # Get the landmark points for the current angle
            lm_a = user_landmarks[ref_angle['a']]
            lm_b = user_landmarks[ref_angle['b']]
            lm_c = user_landmarks[ref_angle['c']]
            weight = ref_angle.get('weight', 1.0)
            ...
            if lm_a.visibility > 0.7 and lm_b.visibility > 0.7 and lm_c.visibility > 0.7:
                ...
                # Check if this angle is the one with the biggest error so far
                if angle_diff * weight > worst_angle_feedback["diff"]:
                    worst_angle_feedback["diff"] = angle_diff * weight
                    worst_angle_feedback["name"] = ref_angle.get('name', 'a key joint')

                # Calculate score for this angle. It's a gentle curve.
                # An error of 40 degrees results in a score of 0 for this angle.
                angle_score = max(0, 1 - (angle_diff / 40))
                total_score += angle_score * weight
        
        # If no joints are clearly visible, return a 0 score.
        if total_weight == 0:
            return 0, "Move your full body into the frame."

        # Calculate the final score as a weighted average
        final_score = (total_score / total_weight) * 100
        
        # Generate the final feedback message
        if final_score > 95:
            feedback_msg = "Perfect Form! Hold the pose."
        elif final_score > 80:
            feedback_msg = "Excellent! A few minor adjustments needed."
        elif final_score > 60:
            # Provide specific feedback on the worst-performing joint
            feedback_msg = f"Good! Focus on adjusting your {worst_angle_feedback['name']}."
        else:
            feedback_msg = f"Needs improvement. Let's work on your {worst_angle_feedback['name']}."
        
        return final_score, feedback_msg
    # ===================================================================
    # END OF THE NEW, IMPROVED FUNCTION
    # ===================================================================
    ...
    def update_feedback(self, score, feedback_msg):
        ...
        if score_int > 80: self.progress_bar.config(style="green.Horizontal.TProgressbar")
        elif score_int > 50: self.progress_bar.config(style="yellow.Horizontal.TProgressbar")
        else: self.progress_bar.config(style="red.Horizontal.TProgressbar")

    def reset_feedback(self):
        self.progress_bar['value'] = 0
        self.progress_bar.config(style="green.Horizontal.TProgressbar") # Default to green
